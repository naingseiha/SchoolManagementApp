...
### ✅ Grades Summary (View Only)
**Path**: `/grades` or tap "ពិន្ទុ" from mobile nav

**Test:**
1. Tap the class selector at top to expand filters
2. Select class, month, year
3. Should see 4 gradient stat cards:
   - Class Average (blue)
   - Highest Score (green)
   - Lowest Score (orange)
   - Pass Rate (purple)
4. Scroll down to see student cards
5. Each student shows:
   - Rank badge (#1, #2, etc.)
   - Name in Khmer
   - Gender
   - Total score / Max score
   - Average (large purple number)
   - Rank position
   - Grade level badge (A, B, C, etc. color-coded)
