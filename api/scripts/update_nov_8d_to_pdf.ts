...
async function main() {
  ...
  console.log(`--- CORRECTLY UPDATING GRADES WITH ROBUST RETRY FOR ថ្នាក់ទី8ឃ (${month} ${year}) (NO STUDENT MOVES) ---`);
  ...
  for (const pdfRow of pdfData) {
    ...
    if (!st || st.classId !== classId) {
      console.log(`ℹ️ Student ID ${pdfRow.studentId} (${pdfRow.name}) not found inside 8ឃ. Skipping!`);
      continue;
    }
  ...
  console.log('\n--- RECALCULATING MONTHLY SUMMARIES FOR ALL CURRENT STUDENTS IN CLASS 8D/8ឃ ---');
