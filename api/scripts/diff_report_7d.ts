...
// 16 subject scores from columns 4 to 19 of PDF table (WRITER, WRITING, MORAL, HIST, GEO, MATH, PHY, CHEM, BIO, EARTH, ENG, HE, HLTH, SPORTS, AGRI, ICT)
const pdfData = [
...
async function main() {
  ...
  // Verify all array lengths
  for (const p of pdfData) {
    if (p.scores.length !== 16) {
      throw new Error(`Row ${p.no} (${p.name}) has scores array length ${p.scores.length} instead of 16!`);
    }
  }
  ...
  console.log('--- SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី7ឃ (វិច្ឆិកា) ---');

  for (const pdfRow of pdfData) {
    const st = students.find(s => s.studentId === pdfRow.studentId);
    if (!st) {
      console.log(`ℹ️ PDF row [${pdfRow.no}] ${pdfRow.name} (${pdfRow.studentId}) is NOT inside 7ឃ in the System (transferred out). Skipping!`);
      continue;
    }
  ...
  console.log(`\n========================================`);
  console.log(`Total active G7-ឃ students checked against PDF: ${activeStudentsChecked}`);
