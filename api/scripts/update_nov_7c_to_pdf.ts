...
async function main() {
  ...
  console.log(`--- UPDATING GRADES WITH ROBUST RETRY FOR ថ្នាក់ទី7គ (${month} ${year}) TO MATCH PDF (NO STUDENT MOVES) ---`);
  ...
  for (const pdfRow of pdfData) {
    ...
    if (!st || st.classId !== classId) {
      console.log(`ℹ️ Student ID ${pdfRow.studentId} (${pdfRow.name}) not found inside 7គ (` + (st ? `is in classId=${st.classId}` : `not in DB`) + `). Skipping per strict instruction!`);
      continue;
    }
  ...
  console.log('\n--- RECALCULATING MONTHLY SUMMARIES FOR ALL CURRENT STUDENTS IN CLASS 7C/7គ ---');
