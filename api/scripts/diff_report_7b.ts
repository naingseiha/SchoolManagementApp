...
async function main() {
  ...
  console.log('--- FINAL EXACT SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី7ខ (វិច្ឆិកា) ---');

  for (const pdfRow of pdfData) {
    const st = students.find(s => s.studentId === pdfRow.studentId);
    if (!st) {
      // Student is not in G7-ខ (e.g. transferred to another class like 25070110 in G7-គ)
      continue;
    }
  ...
  console.log(`\n========================================`);
  console.log(`Total active G7-ខ students checked against PDF: ${pdfData.length}`);
  console.log(`Students with score differences: ${studentsWithDiscrepancies}`);
  console.log(`Total exact score matches: ${totalMatches}`);
  console.log(`Total score discrepancies remaining: ${totalDiscrepancies}`);
  console.log(`========================================`);
}
