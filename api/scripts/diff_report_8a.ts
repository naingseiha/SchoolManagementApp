...
const pdfData = [
  ...
  { no: '0012', studentId: '25080063', name: 'ឈឿម សុខបញ្ញា', scores: [60, 33, 39, 50, 50, 80, 11, 40, 30, 48, 48, 45, 28, 40, 50, 40] }, // Note: class 8B in DB, should be skipped per rules
...
async function main() {
  ...
  console.log('--- SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី8ក (វិច្ឆិកា) ---');

  for (const pdfRow of pdfData) {
    const st = students.find(s => s.studentId === pdfRow.studentId);
    if (!st) {
      console.log(`ℹ️ PDF row [${pdfRow.no}] ${pdfRow.name} (${pdfRow.studentId}) is NOT inside 8ក in the System (transferred out). Skipping!`);
      continue;
    }
  ...
  console.log(`\n========================================`);
  console.log(`Total active G8-ក students checked against PDF: ${activeStudentsChecked}`);
