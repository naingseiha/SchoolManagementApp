...
// 59 rows from PDF OCR
// Each row corresponds exactly to studentId 25070001 through 25070059
const pdfData = [
...
async function main() {
  ...
  const grades = await prisma.grade.findMany({
    where: { classId, month: 'វិច្ឆិកា', year: 2025 }
  });
  ...
  console.log('--- FINAL EXACT SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី7ក (វិច្ឆិកា) ---');
  ...
  console.log(`\n========================================`);
  console.log(`Total students checked against PDF: ${pdfData.length}`);
