...
async function main() {
  ...
  console.log('--- SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី7ង (វិច្ឆិកា) ---');

  for (const pdfRow of pdfData) {
    const st = students.find(s => s.studentId === pdfRow.studentId);
    if (!st) {
      console.log(`ℹ️ PDF row [${pdfRow.no}] ${pdfRow.name} (${pdfRow.studentId}) is NOT inside 7ង in the System (transferred out). Skipping!`);
      continue;
    }
  ...
  console.log(`\n========================================`);
  console.log(`Total active G7-ង students checked against PDF: ${activeStudentsChecked}`);
