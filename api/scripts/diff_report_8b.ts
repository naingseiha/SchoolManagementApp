...
const pdfData = [
  ...
  { no: '0020', studentId: '25080071', name: 'បឿន លីហុង', scores: [40, 5, 44, 16, 0, 70, 1, 0, 40, 35, 25, 50, 15, 20, 25, 0] }, // Note: class 8C in DB, should be skipped per rules
...
async function main() {
  ...
  console.log('--- SCORE DIFFERENCES REPORT FOR ថ្នាក់ទី8ខ (វិច្ឆិកា) ---');

  for (const pdfRow of pdfData) {
    const st = students.find(s => s.studentId === pdfRow.studentId);
    if (!st) {
      console.log(`ℹ️ PDF row [${pdfRow.no}] ${pdfRow.name} (${pdfRow.studentId}) is NOT inside 8ខ in the System (transferred out). Skipping!`);
      continue;
    }
  ...
  console.log(`\n========================================`);
  console.log(`Total active G8-ខ students checked against PDF: ${activeStudentsChecked}`);
