 ...
 */
const getTeachersLightweight = async (req, res) => {
    try {
...
exports.getTeachersLightweight = getTeachersLightweight;
/**
...
const updateTeacher = async (req, res) => {
    try {
        ...
        console.log(JSON.stringify(req.body, null, 2));
        console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
        const { firstName, lastName, khmerName, email, phone, gender, role, employeeId, position, address, dateOfBirth, hireDate, homeroomClassId, subjectIds, teachingClassIds, } = req.body;
        ...
        const result = await prisma.$transaction(async (tx) => {
            ...
            await tx.teacher.update({
                ...
                data: {
                    ...
                    homeroomClassId: role === "INSTRUCTOR"
...
const bulkCreateTeachers = async (req, res) => {
    try {
        ...
        const mapSubjectNamesToIds = (subjectsString) => {
            ...
            return subjectNames
                .map((name) => {
                const subject = allSubjects.find((s) => s.nameKh?.toLowerCase() === name.toLowerCase() ||
        ...
        const mapClassNamesToIds = (classesString) => {
            ...
            return classNames
                .map((name) => {
                const cls = allClasses.find((c) => c.name?.toLowerCase() === name.toLowerCase());
                return cls?.id;
            })
                .filter((id) => id);
        };
        // ✅ Helper: Find homeroom class ID
        const findHomeroomClassId = (className) => {
            if (!className || className.trim() === "")
                return null;
            const cls = allClasses.find((c) => c.name?.toLowerCase() === className.trim().toLowerCase());
            return cls?.id || null;
        };
        ...
        for (let i = 0; i < teachers.length; i++) {
            ...
            try {
                ...
                // ✅ Validate role
                const role = teacherData.role?.toUpperCase() === "INSTRUCTOR"
                    ? "INSTRUCTOR"
                    : "TEACHER";
                // ✅ Map subjects
                ...
                if (role === "INSTRUCTOR") {
                    ...
                    if (!homeroomClassId) {
                        throw new Error(`Homeroom class "${teacherData.homeroomClass}" not found`);
                    }
                    ...
                    if (classWithTeacher?.homeroomTeacher) {
                        throw new Error(`Class "${teacherData.homeroomClass}" already has a homeroom teacher`);
                    }
                ...
                const result = await prisma.$transaction(async (tx) => {
                    ...
                    const teacher = await tx.teacher.create({
                        data: {
                            ...
                            khmerName: teacherData.khmerName?.trim() ||
                ...
                console.log(`  ✅ Row ${rowNumber}: ${result.khmerName} (${result.employeeId})`);
            }
            catch (error) {
                results.failed.push({
                    row: rowNumber,
                    name: teacherData.khmerName ||
                        `${teacherData.firstName || ""} ${teacherData.lastName || ""}` ||
                        "Unknown",
                    error: error.message,
                });
...
exports.bulkCreateTeachers = bulkCreateTeachers;
/**
...
const bulkUpdateTeachers = async (req, res) => {
    try {
        ...
        await Promise.all(teachers.map(async (teacherUpdate) => {
            ...
            try {
                    ...
                    return await tx.teacher.update({
                        ...
                        data: {
                            ...
                            lastName: updateData.lastName !== undefined
                                ...
                                : undefined,
                            khmerName: updateData.khmerName !== undefined
                                ? updateData.khmerName?.trim() || null
                                : undefined,
                            englishName: updateData.englishName !== undefined
                            ...
                            dateOfBirth: updateData.dateOfBirth !== undefined
                                ...
                                : undefined,
                            hireDate: updateData.hireDate !== undefined
                                ? updateData.hireDate
                                : undefined,
                            homeroomClassId: updateData.role === "INSTRUCTOR"
                ...
                results.success.push({
                    ...
                    name: updatedTeacher.khmerName ||
                        `${updatedTeacher.firstName} ${updatedTeacher.lastName}`,
                });
                console.log(`  ✅ Updated: ${updatedTeacher.khmerName || updatedTeacher.firstName}`);
            }
            catch (error) {
                results.failed.push({
                    id,
...
exports.bulkUpdateTeachers = bulkUpdateTeachers;
