...
export class DashboardController {
  ...
  /**
   * Get top students by average score with optional filters
   */
  static async getTopStudents(req: Request, res: Response) {
    try {
      const { month, year, limit = 10, grade, classId } = req.query;
      
      // Parse parameters
      const limitNum = parseInt(limit as string) || 10;
      const academicYear = year ? parseInt(year as string) : new Date().getFullYear();
      
      // Build where clause - Don't filter by month for rankings, use all available data
      ...
      // Add class filter if provided (overrides grade filter)
      if (classId && classId !== 'all') {
        studentWhere.classId = classId as string;
      }
      ...
      grades.forEach(grade => {
        ...
        if (!studentAverages.has(studentId)) {
          studentAverages.set(studentId, {
            id: studentId,
            name: grade.student.khmerName || `${grade.student.firstName} ${grade.student.lastName}`,
            className: grade.student.class?.name || 'N/A',
            gender: grade.student.gender,
            scores: [],
          });
        }
        const student = studentAverages.get(studentId)!;
        student.scores.push(grade.percentage!);
      });
      
      // Calculate averages and sort
      const topStudents = Array.from(studentAverages.values())
        .filter(student => student.scores.length > 0) // Only students with grades
        .map(student => ({
          id: student.id,
          name: student.name,
      ...
      console.log(`🏆 Top ${limitNum} Students (Grade: ${grade || 'all'}, Class: ${classId || 'all'}):`, topStudents.length);
